...
function getClientId() {
  return window.AURALIS_CONFIG?.jamendoClientId || TEST_CLIENT_ID;
}

function genreOf(track) {
  const tags = track?.musicinfo?.tags;
  return tags?.genres?.[0] || tags?.vartags?.[0] || 'Independent';
}
...
async function request(params = {}) {
  ...
  if (json?.headers?.status && json.headers.status !== 'success') {
    throw new Error(json.headers.error_message || 'Jamendo request failed');
  }
...
export const jamendoProvider = {
  ...
  async search(query, limit = 24) {
    ...
    return request({ search: query.trim(), limit, order: 'relevance', boost: 'popularity_month' });
  },
  async featured(tag, limit = 30) {
    return request({ tags: tag, featured: 1, limit, groupby: 'artist_id', boost: 'popularity_month' });
  },
  async popular(limit = 30) {
    return request({ limit, order: 'popularity_week', groupby: 'artist_id' });
  },
  async latest(limit = 30) {
    return request({ limit, order: 'releasedate_desc', groupby: 'artist_id' });
  }
};
